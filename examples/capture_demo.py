...
def main() -> None:
    ...
    try:
        ...
        save(
            ...
            run([*pyimgtag, "preflight", "--input-dir", fixtures, "--ollama-url", MOCK_URL]),
        )
        ...
        # 4. status
        save("status", "pyimgtag status", run([*pyimgtag, "status", *db_flag]))

        # 5. cleanup
        save("cleanup", "pyimgtag cleanup", run([*pyimgtag, "cleanup", *db_flag]))
        ...
        # 9. tags list
        save("tags_list", "pyimgtag tags list", run([*pyimgtag, "tags", "list", *db_flag]))
        ...
        save(
            ...
            run([*pyimgtag, "faces", "scan", "--input-dir", fixtures, *db_flag]),
        )
