...
<body>
  ...
  (() => {
    ...
    async function patchTags(filePath, tags) {
      return api('__API_BASE__/api/images/tags', {
        method: 'PATCH',
    ...
    async function patchCleanup(filePath, cleanupClass) {
      return api('__API_BASE__/api/images/cleanup', {
        method: 'PATCH',
        headers: {'Content-Type': 'application/json'},
    ...
    async function loadStats() {
      try {
        const s = await api('__API_BASE__/api/stats');
    ...
    async function load() {
      ...
      let url = '__API_BASE__/api/images?limit=' + PAGE + '&offset=' + offset;
    ...
    function makeCard(img) {
      ...
      thumb.src = '__API_BASE__/thumbnail?path=' + encodeURIComponent(img.file_path) + '&size=200';
